...
namespace ns3 {
// [ns-3.48 port] Keep per-flow log files open instead of re-opening them on every event
// (re-opening per cwnd/RTT change makes 100 Mbit/s runs orders of magnitude slower).
static std::ofstream& CachedLogStream(const std::string& path) {
    static std::map<std::string, std::unique_ptr<std::ofstream>> streams;
    auto it = streams.find(path);
    if (it == streams.end()) {
        it = streams.emplace(path, std::make_unique<std::ofstream>(path, std::ofstream::out | std::ofstream::app)).first;
    }
    return *it->second;
}
...
{
    std::ofstream& ofs = CachedLogStream(m_baseLogsDir + "/" + format_string("tcp_flow_%" PRIu64 "_cwnd.csv", m_tcpFlowId));
...
{
    std::ofstream& ofs = CachedLogStream(m_baseLogsDir + "/" + format_string("tcp_flow_%" PRIu64 "_rtt.csv", m_tcpFlowId));
...
TcpFlowSendApplication::InsertProgressLog (int64_t timestamp, int64_t progress_byte) {
    std::ofstream& ofs = CachedLogStream(m_baseLogsDir + "/" + format_string("tcp_flow_%" PRIu64 "_progress.csv", m_tcpFlowId));
